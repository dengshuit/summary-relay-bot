...
Bot token、owner id、Telethon userbot 凭据/session、LLM API key、provider model、summary profile 和群组摘要设置都属于数据库管理的运行时配置。数据库 schema 初始化后通过 WebUI 配置。空数据库或没有 enabled bot 时 WebUI 仍会启动；只有成功加载并解密 enabled bot 后才会启动 Telegram polling。
...
| `TELEGRAM_API_PROXY` | 空 | 可选 Telegram Bot API 出站代理。需要代理访问 Telegram 时配置，例如 `socks5://host.docker.internal:7890` 或 `http://proxy:8080`。Userbot 代理在 WebUI 中单独配置，SOCKS 支持需要 `python-socks[asyncio]`。 |
...
Telethon 群摘要重构会把开发数据库 schema 重置为独立的 relay 和 summary 域表。旧 Bot API 群摘要 schema 中的开发数据不会迁移或保留。使用本重构前，请重建或清空本地开发数据库，然后对空数据库执行 `alembic upgrade head`。
...
1. 在 `my.telegram.org` 创建 Telegram application，复制 `api_id` 和保密的 `api_hash`。
2. 打开 WebUI 的 Userbot 页面，配置 userbot 名称、`api_id`、`api_hash`、手机号和可选代理 URL。
3. 发送登录验证码，提交 Telegram 验证码；如果 Telegram 要求 2FA，再提交 2FA 密码。
4. 在 Groups 页面使用 Userbot 刷新动作发现可见群组。
5. 只启用需要摘要的群组。

保存的 Telethon `StringSession` 可以操作该 Telegram 账号。请把它当作密码处理：保持 `SETTINGS_ENCRYPTION_KEY` 稳定且保密，不要把 session 复制到日志或工单；如果泄露，应轮换或撤销 session。

第一版 Telethon 实现只采集 userbot 启动后或重连投递到 update stream 的消息，不会主动调用 `iter_messages` 或 `get_messages` 做历史回填。
...
群摘要管理已迁移到 WebUI。列出群组、启用/禁用摘要、修改间隔和手动摘要不再属于私聊中转 bot 的 Telegram 命令面。
...
3. 调用配置的摘要客户端。
4. 持久化生成的摘要结果，供 WebUI 查看。
5. 结果持久化后，如果游标未被并发修改，则推进游标。
6. 私聊中转 Bot 可用时，异步调度有界通知投递给管理员。

LLM 失败、超时和空输出会保持游标不变。Telegram 通知失败会记录为投递尝试，不会回滚已持久化的摘要结果或游标推进。

定时摘要和手动摘要共用同一个摘要服务和游标逻辑。APScheduler 的 `coalesce` 和 `max_instances=1` 只是本地保护；数据库中的 running-job 约束才是处理重叠任务的正确性边界。

摘要通知会在需要时把完整摘要拆成有序消息块发送。投递最多包含一次初始尝试和两次重试，每次尝试的超时时间是一分钟。
...
2. 在 WebUI 中配置并授权 Telethon userbot。
3. 从 Groups 页面刷新可见群组，确认新发现群组默认禁用。
4. 在 WebUI 启用一个群组并配置摘要间隔。
5. 发送群组消息，并在 WebUI 手动触发摘要。
6. 即使私聊通知不可用，也确认摘要结果出现在 WebUI 中。
7. 私聊中转 Bot 运行时，确认管理员收到摘要通知，并且 WebUI 投递状态更新。
8. 让一个非管理员私聊用户向 bot 发消息。
9. 确认管理员收到信息卡和复制消息。
10. 管理员回复映射到的消息，确认私聊用户收到回复。
11. 对已知私聊用户测试 `/reply <user_id> <message>`。
...
| WebUI 看不到某个群组 | bot 加入群组并具备接收 update 能力后，在该群组发送一条新消息。 |
