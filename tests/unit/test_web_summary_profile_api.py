...
async def test_get_summary_profiles_returns_effective_model_and_does_not_leak_provider_secret(
    ...
    assert [item["name"] for item in payload["items"]] == ["Default profile", "Override profile"]
    assert payload["items"][0]["llm_provider"] == {
        "id": 1,
        "name": "Primary Claude",
        "provider_type": "anthropic",
    }
    assert payload["items"][0]["effective_model"] == "claude-default"
    assert payload["items"][0]["uses_provider_default_model"] is True
    assert payload["items"][1]["effective_model"] == "claude-override"
    assert payload["items"][1]["uses_provider_default_model"] is False
