...
from summary_relay_bot.services.group_settings import group_summary_settings, update_group_summary_interval
...
async def test_discovered_group_has_no_schedule_interval_until_configured(db_session) -> None:
    ...
    assert await group_summary_settings(db_session, group=group) is None
...
async def test_set_interval_does_not_enable_disabled_group(db_session) -> None:
    ...
    settings = await group_summary_settings(db_session, group=updated)
